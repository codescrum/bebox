...
describe 'Phase 02: Bebox::Environment' do
  ...
  describe 'Run vagrant boxes for the environment', :slow do
    ...
    describe 'Configure the hosts file' do
      ...
      it 'should add the hosts config to hosts file' do
        ...
        expect(hosts_content).to include(*subject.project.servers.map{|server| "#{server.ip} #{server.hostname}"})
      end
    ...
    it 'should create Vagrantfile' do
      ...
      output_file = File.read("#{subject.project.path}/Vagrantfile").gsub(/\s+/, ' ').strip
    ...
    it 'should up the vagrant boxes' do
      ...
      nodes_running = true
      subject.project.servers.size.times{|i| nodes_running &= (vagrant_status =~ /node_#{i}\s+running/)}
      expect(nodes_running).to eq(true)
    end

    it 'should connect to config boxes through ssh' do
      connection_successful = true
      subject.project.servers.each do |server|
